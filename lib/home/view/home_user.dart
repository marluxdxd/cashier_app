...
class _UserViewState extends State<UserView> {
  ...
  int calculateTotal(RowData row) {
    ...
    if (row.product!.promo) return price;
    ...
    for (var row in rows) {
      if (row.product != null && row.qty > 0) {
        ...
        final sale = Sale(
          ...
          promoDiscount: row.product!.promo ? 1 : 0,
          date: DateTime.now().toIso8601String(),
        );

        // Save locally + queue Supabase sync
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: AppDrawerUser(onProductAdded: () async => await loadProducts()),
